...
> 이 파일은 장기 기억을 요약하는 문서가 아니다. 새 세션에서 **현재 시스템이 과거 기억과 무엇이 달라졌는지** 빠르게 파악하기 위한 변화량 지도다.
...
- 자동화: 일반 `chatgpt/*` PR은 전체 Actions·리뷰·충돌 게이트 통과 후 자동 병합 가능. 워크플로·Worker·API·Secret·성경/정경 데이터·스키마·lockfile은 승인 게이트를 유지한다.
...
- 문맥 성경: CI 기준 66/66권 curated, coarse 0, fallback 0, Arc 관계 1,262개 검증.
...
- 관찰 카드: 현재 `main` 1,141장. PR #169 병합 후보의 실제 검증값은 1,153장·59권 완성·1권 부분.

## 4. 현재 실제 작업·차단
- 최신 확인 `main`: `34ae12c8bc5913b5a78c608a205848acd95ff13d`
- 최신 main 변경: 모바일 문맥 성경 장 이동 후 바깥 스크롤 안정화.
- PR #169: 히브리서 1–13장 관찰 카드. CI 실패 원인은 A6 집계 불일치.
