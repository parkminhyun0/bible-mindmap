...
- **데이터 전달 표준(A안) · 2026-08-07 확정**: 앱 셸은 Pages, **대용량 정적 데이터는 jsDelivr `@<tag>` CDN 오프로드**(`VITE_DATA_BASE_URL`). 1차 실패 시 **Statically→GitHub raw→앱 동일출처 2차 미러 런타임 폴백**(`src/config/dataBase.js`의 `resilientFetch`/`fetchData`). 데이터 변경은 **새 불변 tag + env 갱신**(tag 재사용 금지). 상세: [[bible-deploy-mechanism]] · [[bible-release-workflow]].
