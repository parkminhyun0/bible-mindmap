...
// 인수를 반드시 준다. 기본값을 두었더니 인수 없이 실행돼 엉뚱한 배치(task11)를
// 병합한 일이 있었다. 잘못 부르면 조용히 다른 데이터를 건드리는 것보다 멈추는 게 낫다.
const taskArg = process.argv[2];
if (!taskArg || !/^task\d+$/.test(taskArg)) {
  console.error('사용: node merge-rulings.mjs <taskNN>   (예: task22)');
  process.exit(1);
}
