...
같은 조작을 backdrop-filter 켠 상태와 끈 상태에서 각각 수행해 프레임 시간을 비교했다.

**캔버스 드래그** (1440×900 · 90프레임)

| 상태 | 중앙값 | p95 | 32ms 초과 | 살아 있는 blur 요소 |
|---|---:|---:|---:|---:|
| 켬(현재) | 8.3 ms | 8.7 | 0 | 9 |
| 끔 | 8.3 ms | 9.2 | 0 | 0 |
...
| 상태 | 모달 열기 | 중앙값 | p95 | 최악 | 32ms 초과 |
|---|---:|---:|---:|---:|---:|
| 켬(현재) | **847 ms** | 8.3 | 9.0 | 41.6 | 1 |
| 끔 | **445 ms** | 8.3 | 9.0 | 24.9 | 0 |

여는 순간만 약 2배(0.4초 차이)다. 전면을 덮는 `.at-modal-backdrop` 블러 레이어를
처음 합성하는 비용이다. 열린 뒤 스크롤은 차이가 없다.

**왜 가설이 틀렸나**

"48건"은 CSS 선언 수였다. `-webkit-` 접두사 쌍이라 선택자는 약 24개이고, 그중 화면에
실제로 살아 있는 것은 9~11개다. 나머지는 모바일 전용이거나 특정 상태에서만 뜬다.
우려했던 `.at-canvas-edge__label` 은 엣지가 없으면 아예 생성되지 않는다.
**"48곳이 상시 비용"이라는 전제가 틀렸다.**

**판단: 착수하지 않는다.** 0.4초를 위해 Apple 시각 파운데이션의 핵심 질감을 걷어내는
것은 남는 거래가 아니다. 손댄다면 `.at-modal-backdrop` 하나가 유일한 후보인데,
그것이 시각적 손실이 가장 큰 지점이기도 하다.

**측정 한계** — 헤드리스 소프트웨어 렌더러다. 실기기는 GPU 로 블러를 처리하므로
절대값이 다르다. 다만 소프트웨어 렌더링은 보통 블러 비용을 과장하는데도 차이가
나오지 않았다는 점은 의미가 있다. 저사양 안드로이드 실기기는 측정하지 못했다.
...
## 3-2. D · 모바일 직접조작 — 대상이 존재하지 않는다 (보류)

패킷 §5 의 D 단계는 "사전 Drawer 또는 보조 Sheet 1개" 에 1:1 드래그·velocity
handoff·momentum/rubber-band·interruptibility 를 적용하는 것으로 잡혀 있다.
그런데 그 대상이 이 저장소에 없다.

| 후보 | 실제 정체 |
|---|---|
| `LexiconTranslationDrawer.jsx` (123줄) | 이름과 달리 **정의 트리 포맷터**. 제스처 코드 0건 |
| `MobileModalFrame.jsx` (134줄) | **전체화면(100dvh) 기하 계약**. 끌어내리는 시트가 아니다 |
| `MobileWorkspaceDock.jsx` (90줄) | 하단 **탭 바**. `pointerdown` 1건은 이벤트 전파 차단용이고 드래그가 아니다 |

모바일 모달은 전부 전체화면이고, 손가락으로 끌어내리는 바텀시트 자체가 없다.
즉 D 는 **개선이 아니라 신규 도입**이다.

### 도입할 경우 충돌하는 기존 계약

- `MobileModalFrame` 의 전체화면 기하 계약
- `useModalDialog.js`(402줄) 의 focus trap · scroll lock · 복귀 생명주기
- `tests/p1-modal-accessibility.spec.js` 가 지키는 모달 계약
- pinch zoom 중 모달이 리사이즈되지 않도록 한 처리 (`MobileModalFrame.jsx` 주석에 명시)

특히 "아래로 쓸어서 닫기" 는 **본문 스크롤과 직접 충돌**한다. 본문을 읽으려 쓸어내렸는데
모달이 닫히면 최악의 오작동이다. 보통 상단 손잡이 영역으로 드래그를 제한해 피하는데,
그러면 "한 손으로 편하게" 라는 원래 이득이 줄어든다.

### 판단 — 다음 순번으로 보류

결함을 고치는 일이 아니라 새 상호작용을 들이는 일이다. 위험 대비 이득이 분명하지 않다.
착수 판단의 기준은 코드가 아니라 **실사용 불편 여부**다 — 한 손으로 모달을 닫기가
실제로 불편한지. 그 확인 없이는 만들지 않는다.
...
| — | — | ~~신규 P1 · `backdrop-filter`~~ → **기각**. 실측 결과 상시 비용 없음 (§3) |
| D · 모바일 직접조작 | 4 | **보류** — 대상 시트가 없어 신규 도입 과제다 (§3-2) |
| E · 모달 모션 계약 | 5 | 유지 |
| F · 디자인 계약 문서 | 6 | 유지 |
| G · Visual/A11y QA | 7 | 유지 — 단 §2 결함을 baseline 에 반드시 포함 |

**착수 순서 현황: ~~P0 고대비 포커스~~ 완료 → ~~P1 backdrop-filter~~ 기각 → ~~D 모바일 직접조작~~ 보류 → E · F · G**
...
- `backdrop-filter` 는 §3 에서 **측정을 마쳤고 가설을 기각했다.** 다만 저사양 실기기는 측정하지 못했다.
