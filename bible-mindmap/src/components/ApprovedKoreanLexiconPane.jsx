...
@media (max-width: 767px) {
  /*
   * WordSearch mobile must have one vertical scroll owner: the modal result area.
   * The desktop dictionary layout uses independently scrollable columns; on iOS
   * those nested scroll regions fight for the touch gesture and can strand the
   * approved Korean pane below the clipped 560px dictionary column.
   */
  .at-modal--word-search div:has(> div > div > [data-word-search-korean-lexicon]) {
    height: auto !important;
    max-height: none !important;
    overflow: visible !important;
  }

  .at-modal--word-search div:has(> div > div > [data-word-search-korean-lexicon]) > div {
    max-height: none !important;
    overflow: visible !important;
    -webkit-overflow-scrolling: auto !important;
  }

  .at-modal--word-search div:has(> div > [data-word-search-korean-lexicon]) {
    height: auto !important;
    max-height: none !important;
    overflow: visible !important;
  }

  .at-modal--word-search div:has(> [data-word-search-korean-lexicon]) {
  ...
  .at-modal--word-search div:has(> [data-word-search-korean-lexicon]) > [data-word-search-korean-lexicon] {
    order: -1;
  }

  .at-modal--word-search div:has(> [data-word-search-korean-lexicon]) > section:first-child {
    max-height: none !important;
    overflow: visible !important;
  ...
  .at-modal--word-search div:has(> [data-word-search-korean-lexicon][data-mobile-korean-open="true"]) > section:first-child {
    display: none !important;
  }

  [data-word-search-korean-lexicon] {
    height: auto !important;
    min-height: 0 !important;
    overflow: visible !important;
  }

  [data-word-search-korean-lexicon] [data-modal-scroll-region="true"] {
    overflow: visible !important;
    max-height: none !important;
...
function MobileLexiconToggle({ koreanOpen, setKoreanOpen, strong }) {
  return (
    <div
      data-word-search-mobile-lexicon-toggle={strong || ''}
      style={{
        display: 'grid',
    ...
    >
      <button
        type="button"
        aria-pressed={!koreanOpen}
        onClick={() => setKoreanOpen(false)}
        style={{
          minHeight: 44,
          borderRadius: 10,
          border: `1px solid ${!koreanOpen ? '#b45309' : '#cbd5e1'}`,
          background: !koreanOpen ? '#fff7ed' : '#fff',
          color: !koreanOpen ? '#92400e' : '#64748b',
          fontSize: 13,
          fontWeight: 800,
          cursor: 'pointer',
        }}
      ...
      <button
        type="button"
        aria-pressed={koreanOpen}
        onClick={() => setKoreanOpen(true)}
        style={{
          minHeight: 44,
          borderRadius: 10,
          border: `1px solid ${koreanOpen ? '#b45309' : '#cbd5e1'}`,
          background: koreanOpen ? '#b45309' : '#fff',
          color: koreanOpen ? '#fff' : '#92400e',
          fontSize: 13,
          fontWeight: 800,
          cursor: 'pointer',
          boxShadow: koreanOpen ? '0 2px 7px rgba(180,83,9,.22)' : 'none',
        }}
      >
        한글 번역
      </button>
...
export default function ApprovedKoreanLexiconPane({ strong }) {
  ...
  return (
    <section
      ...
      data-mobile-korean-open={isMobile && mobileKoreanOpen ? 'true' : 'false'}
      style={{
        minWidth: 0,
        minHeight: 0,
        height: '100%',
    ...
    >
      ...
      {isMobile && (
        <MobileLexiconToggle
          koreanOpen={mobileKoreanOpen}
          setKoreanOpen={setMobileKoreanOpen}
          strong={strong}
        />
      )}

      {showKoreanBody && loading && (
        <div
      ...
      {showKoreanBody && !loading && !approvedEntry && (
        <div
          data-modal-scroll-region="true"
          style={{
            flex: 1,
        ...
        >
          <div style={{
            ...
            {failed
              ...
              : '아직 승인된 한글 사전 항목이 없습니다. 승인된 Strong만 이 영역에 표시됩니다.'}
          </div>
        </div>
      )}
        ...
        <>
          <div style={{
            ...
            <div style={{ minWidth: 0, flex: 1 }}>
              ...
              <div style={{ marginTop: 2, color: '#64748b', fontSize: 10 }}>
                {canonicalStrong} · {identity.lemma} · {identity.transliteration?.korean || identity.transliteration?.scientific}
              </div>
            </div>
          </div>
          ...
          >
            <div style={{
              ...
              한국어 정의 본문은 Approval Registry의 {approvalDescription} 데이터를 사용합니다.
              {enrichment ? ' 어원·TWOT 등은 BDB 원사전 보조정보로 구분해 함께 제공합니다.' : ''}
            </div>

            <div style={{ marginBottom: 15 }} data-testid="word-search-approved-korean-sense-tree">
            ...
            <details style={{ marginTop: 8, borderTop: '1px solid #f1f5f9', paddingTop: 10 }}>
              ...
              <div style={{ marginTop: 6 }}>
                <MetadataCard label="승인 방식">{approvalDescription}</MetadataCard>
                <MetadataCard label="승인자">
                  {approvedEntry.reviewer?.reviewerId || approvedEntry.reviewer?.reviewerType}
                </MetadataCard>
                ...
                <MetadataCard label="Evidence FP">
                  <span style={{ fontFamily: 'monospace', fontSize: 10, wordBreak: 'break-all' }}>
                    {approvedEntry.evidencePacketFingerprint}
                  </span>
                </MetadataCard>
                {enrichment?.sourceVersion && (
                  <MetadataCard label="BDB 보조버전">{enrichment.sourceVersion}</MetadataCard>
                )}
              </div>
            </details>
          </div>
