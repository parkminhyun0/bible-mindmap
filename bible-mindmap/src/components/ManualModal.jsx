...
const SECTIONS = [
  ...
  },
  {
    ...
    content: [
      ...
      {
        type: 'tip',
        text: '원어 데이터가 제공되는 절은 원어 단어가 표시되고, 데이터가 없는 절은 한글 번역본(KRV)만 나타납니다. 모바일에서는 설명·약어·트리가 하나의 세로 스크롤 흐름으로 이어지며, 넓은 트리만 좌우로 스크롤합니다.',
      },
    ],
  },
  {
    ...
    content: [
      {
        type: 'desc',
        text: '오른쪽 사이드바의 🔍 원어 성경 다언어 검색 버튼을 클릭하면 전체화면 검색 팝업이 열립니다. 히브리어·헬라어 원문, 영어 글로스(Gloss), 한글 KRV 세 가지 모드로 성경 전체를 검색하고, 원문·ESV·KRV 본문을 동시에 확인할 수 있습니다.',
      },
      {
        type: 'image',
        src: '/images/wordsearch-demo.png',
        alt: '원어 성경 다언어 검색 — אֱלֹהִים(엘로힘) 검색 예시',
        caption: '▲ 원어 성경 다언어 검색 — 히브리어 אֱלֹהִים(엘로힘) 검색 결과 예시.',
      },
      {
        type: 'list',
        title: '입력 모드 종류',
        items: [
          '원문 입력 (히·헬): 히브리어·헬라어 원문 글자 입력 — 단어형(w)·레마(l) 동시 매칭',
          '영어 입력 (Gloss): love, god 등 영어 글로스로 검색 — 해당 글로스를 포함한 레마 전체 탐색',
          '한글 입력 (KRV): KRV 본문 전체 스캔 — 한글 단어를 포함한 절 단위로 결과 제공',
        ],
      },
      {
        type: 'list',
        title: '결과 탭 — 원문·영어·한글 동시 확인',
        items: [
          '[원문] 탭: 히브리어(WLC)·헬라어(NA28) 본문, 검색어 하이라이트',
          '[영어] 탭: ESV 영어 본문, 검색어 하이라이트',
          '[한글] 탭: KRV 한글 본문, 검색어 하이라이트',
          '각 절 클릭 시 해당 본문을 캔버스 구절 노드로 삽입 가능',
        ],
      },
      {
        type: 'list',
        title: '형태소 필터 칩 & 책별 분포',
        items: [
          '결과 상단 칩(명사·남성·복수·독립형 512 등)을 클릭해 특정 형태론 분류만 필터링',
          '칩 옆 숫자는 현재 책 필터 기준 실제 결과 수 (다시 클릭하면 필터 해제)',
          '오른쪽 도넛 차트에서 성경 책별 분포 확인 — 조각 클릭으로 특정 책만 필터링',
        ],
      },
      {
        type: 'step',
        title: '사용 방법',
        steps: [
          '사이드바에서 🔍 원어 성경 다언어 검색 버튼 클릭',
          '팝업 상단에서 입력 모드 선택 (원문 입력 / 영어 입력 / 한글 입력)',
          '검색창에 단어 입력 후 검색 버튼 클릭 (또는 Enter)',
          '[원문] [영어] [한글] 탭을 전환하며 각 언어 본문으로 용례 확인',
          '형태소 칩 클릭 또는 도넛 차트 조각 클릭으로 결과 필터링',
          '오른쪽 상단 A− / A+ 버튼으로 본문 폰트 크기 조절 (10~50pt)',
        ],
      },
      {
        type: 'tip',
        text: '원문·영어로 검색하면 단어 단위 결과가 나오며, 원문·ESV·KRV 본문이 자동으로 함께 파생됩니다. 한글로 검색하면 KRV 절 단위 결과가 나오고 해당 절의 원어 단어가 역으로 파생됩니다.',
      },
    ],
  },
  {
    ...
    content: [
      {
        type: 'desc',
        text: '문맥 성경 (Context Bible) 은 한 권 전체를 한눈에 읽으면서 원어(히브리·헬라) 담화 구조 · 신학 핵심어 · 거시 논리 흐름을 동시에 볼 수 있게 만든 심층 성경 리더입니다. 사이드바의 "📖 문맥 성경" 버튼으로 진입합니다.',
      },
      {
        type: 'list',
        title: '기본 개념',
        items: [
          '개역한글(KRV) 본문 + STEPBible 원어(스트롱 번호 · 모폴로지) 데이터 실시간 결합',
          '구약: 히브리 담화 마커(와이히·히네·키·헤세드·고엘·축복·맹세) 자동 감지',
          '신약: 헬라 담화 접속사(οὖν·γάρ·ἀλλά·ἵνα·ἄρα·μὴ γένοιτο) 자동 감지',
          '학자적 큐레이팅으로 각 권 특수 구조 마커 오버레이 (창조·톨레도트·아케다·주제 선포·이신칭의 등)',
          '좌측 거터에 4대 섹션 · pivot dot · 논리 arc 시각화',
          '우측 패널에 배경(장르·연도·저자·주제) · 각 장 의제 · 담화 카드 · 신학 핵심어 분포',
        ],
      },
      {
        type: 'image',
        src: '/images/manual/context-bible/1.png',
        alt: '문맥 성경 · 창세기 전체 화면',
        caption: '① 창세기(구약 · 히브리) — 좌측 거시구조 거터에 원역사·아브라함·이삭야곱·요셉 4대 섹션 · pivot dot · 논리 arc / 우측 배경 메타 카드에 장르·연도·저자·주제·각 장 의제',
      },
      {
        type: 'image',
        src: '/images/manual/context-bible/1-1.png',
        alt: '문맥 성경 · 창세기 1:5 담화 카드 + 신학 핵심어 분포',
        caption: '①-1 창세기 1:5 활성 절 → 담화 카드(서사 서두 · 히브리어 신호어 וַיְהִי · 뜻·용례 해설) + 이 절의 신학 핵심어 + 1장 전체 핵심어 분포 (엘로힘 26회 · 하야 17회 등)',
      },
      {
        type: 'image',
        src: '/images/manual/context-bible/2.png',
        alt: '문맥 성경 · 로마서 전체 화면',
        caption: '② 로마서(신약 · 헬라) — 좌측 거시구조 죄와 칭의·칭의의 삶·이스라엘·실천 4대 섹션 / 우측 배경 메타 신약 서신서·바울서신 · 하나님의 의(δικαιοσύνη θεοῦ) 주제',
      },
      {
        type: 'image',
        src: '/images/manual/context-bible/2-1.png',
        alt: '문맥 성경 · 로마서 1:9 담화 카드 + 신학 핵심어 분포',
        caption: '②-1 로마서 1:9 활성 절 → 담화 카드(이유·설명 · 헬라어 신호어 γάρ · 뜻·용례 해설) + 이 절의 신학 핵심어 + 1장 전체 핵심어 분포 (믿음 4회 · 성령 2회 · 은혜 2회 등)',
      },
      {
        type: 'step',
        title: '기본 사용법',
        steps: [
          '사이드바에서 📖 **문맥 성경** 버튼 클릭 → 팝업 창 열림 (드래그로 이동 · 우하단 모서리로 크기 조절)',
          '상단 성경 인덱스 칩(구약·신약 66권 약어) 클릭 → 해당 성경 페이지로 전환 (완성된 성경엔 🔴 red dot)',
          '**JUMP** 장 버튼으로 원하는 장 이동 · **1장 흐름** dot 미니맵으로 절 이동',
          '본문 절 클릭 → 우측 분석 패널에 담화 카드 · 신학 핵심어 · 장 전체 분포 자동 표시',
          '핵심어(엘로힘·믿음·헤세드 등) 클릭 → 성경 전체에서 그 단어 종단 추적',
          '방향키 ↑/↓ 로 절 이동 · ←/→ 로 장 이동',
        ],
      },
      {
        ...
        title: '고급 컨트롤',
        steps: [
          '헤더 [Aa] 폰트 사이즈 컨트롤: 본문·부가·레전드·흐름·분석·배경 6개 영역별 독립 조절',
          '헤더 [히] / [헬] 토글: 원어 절 번호 병기 (예: KRV 창 32:1 ↔ 히 32:2, Rom 3:25 ↔ 헬 3:26)',
          '좌·우 패널 사이 스플리터 드래그로 좌우 폭 자유 조절',
          '좌측 거시구조 Arc를 클릭/탭하면 연결 기준·본문 근거·신학적 의미·해석 주의가 담긴 해설 카드 표시',
          '배경 메타 카드 상단 [▼] 버튼으로 접기/펼치기',
        ],
      },
      {
        type: 'list',
        title: '담화 마커 색상 · 아이콘 (좌측 레전드)',
        items: [
          '구약: 📜 서사 서두 (וַיְהִי) · 👁️ 주의 환기 (הִנֵּה) · 💡 이유·설명 (כִּי) · 🕊️ 축복 정형구 · ⚖️ 맹세·언약 · 💠 고엘·기업 무름 · 🤝 헤세드',
          '신약: ❓ 수사적 질문 (τί οὖν) · ⛔ 강한 반박 (μὴ γένοιτο) · 🏁 대 결론 (ἄρα) · ✅ 결론·적용 (οὖν) · ↔ 대조·전환 (ἀλλά) · 💡 이유·설명 (γάρ) · 🎯 목적 (ἵνα)',
          '특수 구조 마커: 창세기 8종(창조·원복음·톨레도트·언약적 부르심·믿음의 의·아케다·이스라엘 이름·하나님의 예비) · 로마서 5종(주제 선포·이신칭의·절정·송영·실천의 부름) · 룻기 4종(언약적 결단·거부·성취·계보 결론)',
        ],
      },
      {
        type: 'tip',
        text: '진행 상황 파악: 성경 인덱스 칩 우상단의 🔴 red dot 이 붙은 칩만 현재 완성된 성경입니다. 클릭 시 완전한 분석 페이지가 열리고, 미완성 칩은 placeholder(본문 준비 중) 페이지가 열립니다.',
      },
      {
        type: 'tip',
        text: '원어 절 번호 병기: 히브리 정본(마소라)과 크리스천 절 구분(스테파누스, 1551)이 다른 지점이 있습니다. 창세기 32장은 전체 33절이 KRV/영어와 1절씩 오프셋되고, 로마서 3:25도 헬라 원어로는 3:26입니다. 헤더의 [히]/[헬] 토글로 원어 번호를 subscript 로 병기할 수 있습니다.',
      },
      {
        type: 'desc',
        text: '🔗 관주(Crossref) — 인용·참조 구절 팝업 · 66권 자동 지원. 각 절 좌측의 🔗 아이콘을 클릭하면 해당 절의 인용·참조 구절 상위 5개가 votes(공동체 투표) 정렬로 표시됩니다. 관주 성경의 상호 참조 기능을 앱 안에서 즉시 확인할 수 있습니다.',
      },
      {
        type: 'image',
        src: '/images/manual/context-bible/crossref-popup.jpg',
        alt: '관주 팝업 · 창세기 1:1 인용·참조 구절 · 카테고리 dot 컬러',
        caption: '③ 관주 팝업 — 창세기 1:1 → 요한복음 1:1-3(🟢 복음서·행전) · 히브리서 11:3(🔵 서신) · 이사야 45:18(🔴 예언서) · 요한계시록 4:11(⚫ 계시록) · 히브리서 1:10(🔵 서신). 정경 카테고리 7종 컬러 dot 으로 한눈에 구분',
      },
      {
        type: 'list',
        title: '정경 카테고리 dot 컬러 (7종)',
        items: [
          '🟡 오경 (Torah): 창·출·레·민·신',
          '🟠 역사서: 수·삿·룻·삼상하·왕상하·대상하·스·느·에',
          '🟣 시가서·지혜: 욥·시·잠·전·아',
          '🔴 예언서: 사·렘·애·겔·단 + 12소예언서',
          '🟢 복음서·행전: 마·막·눅·요·행',
          '🔵 서신: 롬·고전~유 21권',
          '⚫ 계시록: 계',
        ],
      },
      {
        ...
        title: '관주 사용법',
        steps: [
          '문맥 성경 본문에서 각 절 좌측의 🔗 아이콘 클릭 → 관주 목록 팝업 오픈',
          '팝업 상단 카테고리 범례 리본으로 색상 의미 즉시 파악',
          '각 참조 카드 클릭 → **본문 미리보기 팝업** 오픈 (KRV 본문 자동 로드)',
          '**여러 참조 동시 오픈 가능** — 각 팝업 개별 ✕ 닫기 · 화면에 나란히 배치',
          '데스크톱: 팝업 헤더 잡고 **드래그로 자유 이동** (관주 목록·본문 미리보기 모두)',
          '모바일: 하단 시트로 열림 · 배경 dim 탭 시 닫힘',
        ],
      },
      {
        type: 'list',
        title: '본문 미리보기 표기 규칙',
        items: [
          '첫 절만 "장:절" 형식 (예: 1:1) · 카테고리 컬러 tint 배지',
          '이후 같은 장은 "절"만 (예: 2, 3, 4 ...) · 관주 성경 관례',
          '카테고리 컬러가 팝업 상단 border 로 표시되어 어느 정경인지 즉시 인지',
        ],
      },
      {
        type: 'tip',
        text: '학문적 근거: 관주 데이터는 openbible.info 의 커뮤니티 vote 기반 상호 참조(66권 전체). votes 값이 높을수록 신학적으로 널리 인정된 참조입니다. 마가복음 1:2-3 (세례요한) 은 이사야 40:3 · 말라기 3:1 · 출애굽기 23:20 을 결합 인용한 사례처럼, 원저자의 구약 인용 계보를 즉시 추적할 수 있습니다.',
      },
    ],
  },
  {
    ...
    content: [
      {
        type: 'desc',
        text: '⇄ 병렬 본문 연구는 공관복음 평행 · 신약↔구약 인용 · OpenBible 관주를 한 화면에 나란히 놓고, "무엇에 주목할지"와 "어떤 순서로 관찰할지"를 학습 스캐폴딩(가이드 코스 · 연구 렌즈 · 관찰 카드 · 용어 툴팁)으로 안내하는 심층 연구 도구입니다. 사이드바 📖 문맥 성경 바로 아래 "⇄ 병렬 연구" 버튼으로 진입합니다.',
      },
      {
        type: 'list',
        title: '핵심 기능 · 무엇을 볼 수 있는가',
        items: [
          '기준 본문 + 비교 본문 최대 4개를 나란히 배치 · KRV / ESV / 원어(SBLGNT·OSHB) 3개 탭 동시 지원',
          '문자열 diff 자동 표시 — 공통(초록) · 추가(파랑) · 생략(빨강) · 어순(노랑) 4색 마킹',
          '원어 안전장치 — 히브리 OT ↔ 헬라 NT 는 직접 문자열 비교 차단(전승 언어가 달라 무의미). OT↔OT · NT↔NT 만 허용',
          '자동 비교는 문자열 정렬 보조일 뿐, 해석 결론이 아닙니다. curated 관찰과 원어 형태론·구문·본문비평을 함께 확인하세요',
        ],
      },
      {
        type: 'list',
        title: '데이터 소스 3가지',
        items: [
          'curated 공관복음 세트 11개 — 마·막·눅 3본문 세트 + 저자별 편집 관찰(emphasis)',
          'curated 신약↔구약 인용 32개 — 성취 공식 · 자유 인용 · 인유 · 반향 · 유형론 분류',
          'OpenBible 관주(66권 전체) — 커뮤니티 votes 기반 상위 12개 자동 로드',
        ],
      },
      {
        type: 'step',
        title: '기본 사용법',
        steps: [
          '사이드바 "⇄ 병렬 연구" 버튼 클릭 → 팝업 열림 (데스크톱: 헤더 드래그 이동 · 우하단 리사이즈)',
          '최초 실행 시 3단계 온보딩 코치마크 자동 표시 — 기준 본문 → 비교 본문 추천 → 연구 렌즈 순서 안내',
          '① 기준 본문 입력 (예: "마가복음 1:2-3") → [불러오기] 클릭',
          '② 우측 "비교 본문 추천 · 최대 4본문" 칩에서 관계 종류(공관/인용/암시/반향/관주) 확인 후 클릭해 슬롯에 추가',
          '③ 상단 번역 탭(개역한글·ESV·원어)에서 비교 언어 선택 → 색 표시로 차이 즉시 확인',
          '④ 데스크톱은 4열 그리드 · 모바일은 상단 탭으로 본문 전환 (기준 본문은 항상 첫 슬롯에 고정)',
        ],
      },
      {
        type: 'desc',
        text: '🎓 가이드 학습 코스 — 12개 완성 코스로 "무엇을 발견할 수 있는지" 즉시 체험할 수 있습니다. 팝업 상단 캐러셀에서 코스를 클릭하면 기준 본문·비교 본문·추천 렌즈가 자동 세팅되고, 학습 목표 3개 + 4단계 진행 스텝 + 핵심 요지가 활성 코스 패널에 표시됩니다.',
      },
      {
        type: 'list',
        title: '코스 난이도 배지 (12개)',
        items: [
          '입문(초록) 4개 · 25분 안팎 — 세례 요한 · 예수의 세례 · 씨뿌리는 자 · 오병이어',
          '중급(주황) 5개 · 30-40분 — 이사야 성취 인용 클러스터 · 시편 22편 십자가 서사 · 마태 성취 공식 · 마지막 만찬 4중 비교 · 예루살렘 입성',
          '고급(빨강) 3개 · 45-50분 — 히브리서의 시편 110편 재읽기 · 로마서 9-11장 구약 인용 · 요한 서문 반향 후보 조사',
        ],
      },
      {
        type: 'desc',
        text: '🔬 연구 렌즈 — 같은 본문을 "무엇에 주목할지" 8개 렌즈가 잡아줍니다. 렌즈를 선택하면 관찰 프롬프트 3개 + 단계별 워크플로우 4개 + watchOut 경고 + 해당 학술 용어가 하단에 펼쳐집니다. 코스가 활성 상태이면 그 코스가 추천하는 렌즈에 ★ 표시가 붙습니다.',
      },
      {
        type: 'list',
        title: '연구 렌즈 8종',
        items: [
          '✂️ 편집 렌즈 (중급) — 각 저자가 무엇을 넣고·빼고·재배치했는가 (편집비평)',
          '🔤 어휘·용어 렌즈 (입문) — 같은 사건을 각 저자가 어떤 핵심 단어로 말하는가',
          '🧾 인용 기법 렌즈 (고급) — 신약이 구약을 어떻게 선택·변형·결합했는가 (MT·LXX·신약 차이)',
          '🧭 유형론 렌즈 (중급) — 옛 인물·사건·제도가 새 사건에서 어떻게 반복·완성되는가',
          '🌊 반향·인유 렌즈 (고급) — 명시 인용이 아닌 단어·이미지·장면의 메아리 추적',
          '👥 청중·상황 렌즈 (중급) — 같은 전승이 서로 다른 독자·정황에서 어떻게 들리는가',
          '🧩 구조·배열 렌즈 (중급) — 같은 단락이 책의 어디에 놓였고 무엇과 연결되는가',
          '✝️ 신학 강조 렌즈 (고급) — 관찰된 차이가 기독론·구원·교회·소망에 무엇을 더하는가',
        ],
      },
      {
        type: 'desc',
        text: '📖 자동 관찰 카드 — 기준 본문이 curated 세트/인용과 매칭되면 카드가 자동으로 뜹니다. 공관 세트는 연구 질문 3개 + 예상 발견 3개 + "다음 시도" 안내 / 인용은 인용 유형(성취 공식·자유 인용·인유·반향·유형론) + 본문 전승 노트(MT·LXX 차이) + 관찰할 것 3개 + 신학적 지렛대를 제시합니다.',
      },
      {
        type: 'list',
        title: '용어 툴팁 · 30개 학술 용어',
        items: [
          '렌즈·코스·카드 텍스트에 나오는 학술 용어(편집비평·양식비평·자료비평·유형론·인유·반향·상호본문성·미드라쉬·페세르·MT·LXX·chiasm·inclusio 등 30항)에 자동으로 점선 밑줄',
          '마우스 호버 또는 탭 시 툴팁 팝오버 — 다른 이름 · 짧은 정의 · 예시 · ⚠ 주의 노트 표시',
          '입문/중급/고급 난이도 표시로 학습 진행 정도에 맞춰 확장',
        ],
      },
      {
        type: 'step',
        title: '심화 연구 워크플로우 (30분 예시 · 편집 렌즈)',
        steps: [
          '🎓 코스 캐러셀에서 "세례 요한을 통해 배우는 공관대조 입문" 클릭 → 마가 1:1-8 기준으로 마 3:1-12 · 눅 3:1-18 자동 로드',
          '🔬 ✂️ 편집 렌즈 선택 → 관찰 프롬프트 3개(고유 표현 → 청중 효과 → 신학 강조 연결)와 4단계 워크플로우 표시',
          '① 워크플로우 1단계 지시대로 세 본문의 첫 문장과 마지막 문장을 먼저 나란히 읽는다',
          '② 상단 KRV 탭에서 색 diff(초록 공통·파랑 추가·빨강 생략·노랑 어순)로 반복 패턴 찾기',
          '③ 하단 자동 공관 카드의 "예상 발견"과 자기 관찰 대조 → 겹치는 부분과 새로 찾은 부분 구분',
          '④ 활성 코스 패널의 4단계 스텝을 순서대로 클릭하며 완료 체크 → 핵심 요지 확인 → 관찰 노트 3문장 정리',
        ],
      },
      {
        type: 'tip',
        text: '반향·인유는 낮은 확신도를 정직하게 표시하세요. 반향 렌즈의 워크플로우는 "직접 인용 / 강한 인유 / 가능한 반향 / 약한 유사" 4단계 확신도 분류를 강제합니다. 겉모양이 비슷하다는 이유만으로 유형론이나 반향을 확정하지 마세요 — 희귀 어휘 · 문맥 적합성 · 주제 연속성이 함께 있어야 합니다.',
      },
      {
        type: 'tip',
        text: '원어 탭 안전장치: 히브리 OT ↔ 헬라 NT 는 자동 문자열 비교가 차단됩니다. 두 본문이 서로 다른 언어 전승이므로 diff 표시가 무의미하기 때문입니다. 대신 두 본문을 병렬로 제시만 하고, 인용 카드의 "본문 전승 노트(MT · LXX · 신약 인용 관계)"를 함께 읽으세요. OT↔OT · NT↔NT 는 정상 diff 됩니다.',
      },
      {
        type: 'tip',
        text: 'curated 관찰과 자동 결론을 구분: 공관 emphasis · 인용 관찰 카드 · 렌즈 프롬프트는 사람이 큐레이팅한 관찰이며, 절대적 결론이 아닙니다. 반드시 "관찰된 차이 → 가능한 저자 의도 → 확신도" 세 단계로 나누어 노트하는 훈련이 병렬 연구의 핵심입니다.',
      },
    ],
  },
  {
