...
- 현재 활성 잔여 항목은 P2 2건, P3 4건이며 운영 차단 결함이 아니라 라이브 검증 대기·제한된 원어 검토·명시적 본문 예외·개발/성능 부채다.
...
### A6-R002 · P2 · 정경 용례지도 72개 완전성·라이브 검증

정경 개념 72개 모두 상세 usage map을 보유하며 총 513개 용례가 등록됐다. 개념·usage 양방향 완전성, Strong, 실제 절 범위, note 길이와 동일 개념 내부 ref 중복을 CI 하드 게이트로 검사한다. main 병합 이후 실제 UI·모바일·Pages 라이브 확인이 남아 있다.
...
3. 정경 개념 72개·usage map 72개·대기 0개의 실제 registry 일치
