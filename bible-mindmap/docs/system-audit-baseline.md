...
감사 완료 판단 기준 커밋: `0c550ae60d0e4351601f011fccf354384c682faa`
...
| A4 | 검색·NVIDIA 품질 | Hybrid·차원·모델·오탐·출처 평가 통과 | 완료 · PR #89 + 실제 run `30842224158` |
| A5 | 보안·배포·라이브 | Secret 경계, dependency audit, production build, Pages smoke 통과 | 완료 · PR #90 |
| A6 | 잔여 문제 정리 | P0/P1 0건, 낮은 위험 항목 문서화 | 완료 · PR #91 |

**전체 감사 완료율: 100% · A0~A6 완료**
...
8. NVIDIA PoC가 4문서·4직접질의만 사용해 1.00 점수가 과도하게 쉬우며 오탐을 측정하지 않음 — **A4 수정·실측 완료**, 12문서·16질의·16 hard-negative로 확장하고 실제 endpoint 비교 완료.
...
## A4 검색·NVIDIA 품질 감사 완료 근거

- `canonical-audit-v2`: 승인 문서 12건·출처 36건·질의 16건·hard-negative 16건
- 직접 질의·의미 바꿔쓰기·multi-hop을 segment별 평가
- keyword baseline·vector-only·Hybrid를 분리해 Embedding의 실제 기여 확인
- GitHub Actions run `30842224158`에서 실제 NVIDIA endpoint로 2048/384차원 병렬 실측
...
- multi-hop Recall@3는 2048이 1.0, 384가 0.83333
- 384는 저장량 81.25% 절감이 가능하지만 검색 품질과 오탐 통제가 회귀해 2048차원 유지 결정
- `docs/evidence/nvidia-embedding-dimension-bakeoff-30842224158.json`에 artifact ID·digest·지표를 영구 보존
- 운영 DB와 production 인덱스는 변경하지 않으며 실제 도입은 별도 승인과 구현 PR을 요구
...
- Pages run `30839522394`와 A6 이후 run `30841263158`의 build·deploy·live verification·최종 pipeline 상태 성공
...
- `docs/system-audit-residual-risks.md`에 P0/P1 0건과 활성 P2/P3 후속 조건 설명
- `verify-system-audit-residual-risks.mjs`가 실제 registry·원어 marker 출력·명시적 성경 예외·NVIDIA 실측 근거를 대조
- NVIDIA 실측 대기 A6-R001은 run `30842224158`로 해결됐고 활성 잔여 항목은 P2 2건·P3 4건
- 등록 수치가 개선되거나 악화돼도 근거 문서와 함께 검토하지 않으면 CI 실패
- PR #91 최종 run `30841051750`에서 security-audit·전체 build·A6 verifier·Chromium smoke 11개 성공
