...
| A3 | 모바일·접근성 | 주요 viewport, 키보드, 포커스, 스크롤 계약 통과 | 진행 중 |
...
7. 사용자 매뉴얼의 초기 포커스·Tab 순환·Escape·모바일 스크롤 잠금·포커스 복귀 공백 — **A3 처리 중**.
...
- Escape 닫기와 실행 버튼 포커스 복귀
- 모바일 body·html overflow/overscroll 잠금과 원상 복구
- 자료 추가 시트를 언마운트하지 않고 매뉴얼을 상위 레이어로 유지
- 모바일 viewport 안에 전체 매뉴얼이 유지되는지 검증
- 기존 드래그·리사이즈·최소화·가로 내비게이션·본문 내부 스크롤 보존
- 문맥 성경·병렬 연구·매뉴얼 접근성 회귀를 같은 Chromium smoke 묶음으로 지속 실행
- 오래된 PR #70은 최신 main 기반 A3 PR로 대체한 뒤 종료
