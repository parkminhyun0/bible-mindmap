...
const RESTRICTED_LICENSE_STATUSES = new Set(['unknown', 'prohibited']);
...
function validateRegistry(registry) {
  ...
  if (governance.unknownLicenseBlocksAiInput !== true) {
    fail('unknown licenses must block AI input');
  }
  if (governance.unknownLicenseBlocksRedistribution !== true) {
    fail('unknown licenses must block redistribution');
  }
  ...
  for (const [index, source] of registry.sources.entries()) {
    ...
    if (!Array.isArray(source.languages) || source.languages.length === 0) {
      ...
      if (new Set(source.languages).size !== source.languages.length) {
        fail(`${label}: duplicate language`);
      }
    ...
    if (!Array.isArray(source.sourceTypes) || source.sourceTypes.length === 0) {
      ...
      if (new Set(source.sourceTypes).size !== source.sourceTypes.length) {
        fail(`${label}: duplicate sourceType`);
      }
    ...
    } else if (provenance.datasetPaths.some((value) => !nonEmpty(value))) {
    ...
    if (RESTRICTED_LICENSE_STATUSES.has(license.status)) {
      ...
      if (workflow.autoProcessingAllowed) {
        fail(`${label}: restricted license cannot auto process`);
      }
      if (!allPermissionsDenied(license)) {
        fail(`${label}: restricted license must deny storage, LLM input, derivatives and redistribution`);
      }
      warn(`${label}: license unresolved and correctly blocked`);
    }
    ...
    if (workflow.status === 'approved-ready') {
      ...
      if (!nonEmpty(provenance.version)) fail(`${label}: ready source requires pinned version`);
    ...
    if (workflow.status === 'approved-pending-fingerprint') {
      ...
      if (provenance.contentHash !== null) fail(`${label}: pending fingerprint contentHash must be null`);
      if (!Array.isArray(provenance.datasetPaths) || provenance.datasetPaths.length === 0) {
        fail(`${label}: pending fingerprint requires datasetPaths`);
      }
      warn(`${label}: license approved, source version/fingerprint still pending`);
    }
  ...
  for (const sourceId of governance.sourceTextPrecedence || []) {
    ...
    if (source?.workflow.status === 'blocked') {
      fail(`sourceTextPrecedence cannot include blocked source ${sourceId}`);
    }
  }
...
function runSelfTests(registry) {
  ...
  assert.ok(
    validateRegistry(restrictedButEnabled).errors.some((message) => message.includes('must use blocked workflow')),
  );
...
const tagnt = expectSource(registry, 'stepbible-tagnt');
assert.equal(tagnt.license.expression, 'CC-BY-4.0');
assert.ok(tagnt.languages.includes('greek'));
assert.equal(tagnt.canon, 'new-testament');

const tahot = expectSource(registry, 'stepbible-tahot');
assert.equal(tahot.license.expression, 'CC-BY-4.0');
assert.ok(tahot.languages.includes('hebrew'));
assert.ok(tahot.languages.includes('aramaic'));
assert.equal(tahot.canon, 'old-testament');

const enhancedBdb = expectSource(registry, 'unfoldingword-bdb-enhanced');
assert.equal(enhancedBdb.license.status, 'approved');
assert.equal(enhancedBdb.workflow.status, 'approved-pending-fingerprint');
...
console.log(
  `✓ lexicon source registry gate 통과 · sources=${registry.sources.length} · ready=${ready} · pendingFingerprint=${pending} · blocked=${blocked}`,
);
