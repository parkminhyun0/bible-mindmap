...
export function loadNvidiaTransportConfig(env = process.env) {
  ...
  if (!Number.isFinite(timeoutMs) || timeoutMs < 1_000 || timeoutMs > 120_000) throw new Error('NVIDIA_TIMEOUT_MS must be between 1000 and 120000');
...
export async function createNvidiaChatCompletion({ messages, temperature = 0.1, maxTokens = 1200, requestId, fetchImpl = globalThis.fetch, env = process.env }) {
  ...
  try {
    ...
    let body;
    try { body = text ? JSON.parse(text) : {}; } catch { throw new Error(`NVIDIA response was not JSON (HTTP ${response.status})`); }
    if (!response.ok) throw new Error(`NVIDIA request failed: ${body?.error?.message || body?.message || `HTTP ${response.status}`}`);
    const content = body?.choices?.[0]?.message?.content;
    if (typeof content !== 'string' || !content.trim()) throw new Error('NVIDIA response did not contain message content');
    return { provider: 'nvidia', model: config.model, requestId: body?.id || requestId || null, content, usage: body?.usage || null, raw: body };
