...
let clock = 0;
const report = await runNvidiaEmbeddingPoc({
  embed,
  env,
  now: () => {
    clock += 2;
...
assert(!('NVIDIA_MODEL_ID' in env), 'embedding PoC fixture must not require a chat model id');
assert(report.embedding.model === env.NVIDIA_EMBEDDING_MODEL_ID, 'report must preserve the embedding model id');
assert(report.embedding.dimension === 8, 'report must preserve vector dimension');
assert(report.corpus.count === 4 && report.corpus.sourceRefs === 12, 'PoC corpus size and source count must be explicit');
assert(report.candidate.recallAtK === 1, 'mock PoC Recall@3 must equal 1');
assert(report.candidate.mrr === 1, 'mock PoC MRR must equal 1');
assert(report.candidate.ndcgAtK === 1, 'mock PoC nDCG@3 must equal 1');
assert(report.gate.passed === true, 'mock PoC must pass the retrieval quality gate');
assert(report.existingDbModified === false, 'PoC report must guarantee no existing DB mutation');
