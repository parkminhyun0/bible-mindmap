...
async function resolvePrimaryUrl() {
...
function validatePrimaryUrl(primaryUrl) {
  if (!primaryUrl) return;
  let parsed;
  try { parsed = new URL(primaryUrl); } catch {
    throw new Error(`VITE_VISITOR_API_URL is not a valid URL: ${primaryUrl}`);
  }
...
async function main() {
  const primaryUrl = await resolvePrimaryUrl();
  validatePrimaryUrl(primaryUrl);
  ...
  const partial = rawPartial.replaceAll(URL_PLACEHOLDER, primaryUrl);

  if (primaryUrl) {
    console.log(`▸ primary backend (Cloudflare) = ${primaryUrl}`);
    console.log('▸ secondary backend = CounterAPI v2 public workspace (fallback)');
  } else {
    console.log('▸ primary backend not configured (VITE_VISITOR_API_URL empty)');
    console.log('▸ secondary backend = CounterAPI v2 public workspace (sole source)');
  }
