...
// R4 must NOT be auto-eligible for Approval Registry promotion. It first enters
// EXTENDED_RESEARCH_REQUIRED; evidence-resolvable interpretation differences can
// be decided there without prematurely escalating to a human exception.
assert.equal(matrix.tiers.R4.autoEligibilityRequires.length, 0, 'R4 must have empty auto-eligibility (never auto-promote)')
...
assert.equal(v4.status, 'FOUNDATION_INSTALLED', 'v4 foundation must reflect merged installation truth')
assert.equal(v4.installedByPr, 299, 'v4 foundation install PR must remain pinned to #299')
assert.equal(v4.installedByMergeCommit, 'f07a847317a052148c08de316cea922dc90e61cf', 'v4 foundation merge commit drift')
for (const [key, value] of Object.entries(v4.automationReadiness)) {
  assert.equal(value, 'INSTALLED', `v4 readiness ${key} must be INSTALLED`)
}
...
// ── 7. Post-P5 R3/R4 state must reflect merged truth. The legacy `status`
//     stays unchanged for backward compatibility, while postAuditStatus carries
//     the current v4 progression.
...
assert.equal(p5.postAuditStatus, 'R3_CONSENSUS_COMPLETE_R4_EXTENDED_RESEARCH_REQUIRED', 'postAuditStatus must route R4 to extended research before any human exception')
...
console.log('✓ Lexicon v4 foundation contract · installed=#299 · matrix=AND-strict · R4=EXTENDED_RESEARCH_REQUIRED · triggers=6 fail-closed · TRACK_STATE=reconciled')
