...
assert(gate.passed === true, 'quality gate must pass the approved corpus');
...
expectThrow(
  ...
  /hardNegativeRate/,
  'quality gate must reject excessive hard-negative hits',
);
...
console.log(
  `✓ retrieval corpus verified · cases ${candidate.caseCount} · types ${queryTypes.size} · hard-negative coverage ${(hardNegativeCoverage * 100).toFixed(0)}% · Recall@${config.k} ${candidate.recallAtK.toFixed(2)} · MRR ${candidate.mrr.toFixed(2)} · nDCG@${config.k} ${candidate.ndcgAtK.toFixed(2)} · hard-negative ${(candidate.hardNegativeRate * 100).toFixed(0)}% · failure ${(candidate.failureRate * 100).toFixed(0)}%`,
);
