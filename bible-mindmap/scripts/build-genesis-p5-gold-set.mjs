...
export const GROUP_QUOTAS = Object.freeze({
  'golden-control': 1,
  'core-theology-context': 9,
  'high-frequency-general': 5,
  'medium-frequency-general': 5,
  'low-frequency-general': 5,
})

// Curated Genesis-facing theological/contextual controls. Ordering is deliberate and
// stable; the selector only admits members that actually occur in the rebuilt Genesis inventory.
...
function strongNumber(strong) {
  const match = String(strong || '').match(/^H(\d+)$/)
  return match ? Number.parseInt(match[1], 10) : Number.POSITIVE_INFINITY
}

function compareStrong(a, b) {
  return strongNumber(a.strong || a) - strongNumber(b.strong || b)
}

function compareFrequencyDesc(a, b) {
  return b.occurrences - a.occurrences || compareStrong(a, b)
}

function compareFrequencyAsc(a, b) {
  return a.occurrences - b.occurrences || compareStrong(a, b)
}

function summarizeEntry(entry, group, rank) {
  return {
    strong: entry.strong,
    group,
    rank,
    ...
    legacyCoverage: entry.coverage,
  }
}

function takeOrThrow(items, count, label) {
  if (items.length < count) throw new Error(`${label}: need ${count}, found ${items.length}`)
...
export function buildGenesisP5GoldSet(inventory, approvalRegistry) {
  ...
  if (approvedH776.approvedSenseTree?.length !== 26) {
    throw new Error(`GEN-1-1-H776 must preserve 26 approved senses; found ${approvedH776.approvedSenseTree?.length || 0}`)
  }
  ...
  const corePool = CORE_STRONGS
    .filter((strong) => strong !== GOLDEN_CONTROL && byStrong.has(strong))
    .map((strong) => byStrong.get(strong))
  const core = takeOrThrow(corePool, GROUP_QUOTAS['core-theology-context'], 'core-theology-context')
  for (const [index, entry] of core.entries()) {
    selected.push(summarizeEntry(entry, 'core-theology-context', index + 1))
    selectedStrong.add(entry.strong)
  }

  const generalPool = inventory.entries.filter((entry) => !coreSet.has(entry.strong) && !selectedStrong.has(entry.strong))
  const high = takeOrThrow(
    generalPool.filter((entry) => entry.occurrences >= 20).sort(compareFrequencyDesc),
    GROUP_QUOTAS['high-frequency-general'],
    'high-frequency-general',
  )
  const medium = takeOrThrow(
    generalPool.filter((entry) => entry.occurrences >= 5 && entry.occurrences <= 19).sort(compareFrequencyDesc),
    GROUP_QUOTAS['medium-frequency-general'],
    'medium-frequency-general',
  )
  const low = takeOrThrow(
    generalPool.filter((entry) => entry.occurrences <= 4).sort(compareFrequencyAsc),
    GROUP_QUOTAS['low-frequency-general'],
    'low-frequency-general',
  )

  for (const [group, items] of [
    ['high-frequency-general', high],
    ['medium-frequency-general', medium],
    ['low-frequency-general', low],
  ]) {
    for (const [index, entry] of items.entries()) {
      selected.push(summarizeEntry(entry, group, index + 1))
      selectedStrong.add(entry.strong)
    }
  }

  if (selected.length !== GOLD_SIZE || selectedStrong.size !== GOLD_SIZE) {
    throw new Error(`Gold Set must contain ${GOLD_SIZE} unique Strong IDs; got ${selected.length}/${selectedStrong.size}`)
  }
  ...
  const result = {
    ...
    selectionPolicy: {
      policyId: 'public-first-genesis-representative-v1',
      deterministic: true,
      quotas: GROUP_QUOTAS,
      frequencyBands: {
        high: 'occurrences >= 20',
        medium: '5 <= occurrences <= 19',
        low: 'occurrences <= 4',
      },
      generalGroupsExcludeCuratedCoreList: true,
    },
    sourceEvidence: {
    ...
    governance: {
      ...
      phaseTransitionEffectiveOnlyAfterIndependentReview: true,
    },
    counts: {
      selected: selected.length,
      groups: Object.fromEntries(Object.keys(GROUP_QUOTAS).map((group) => [group, selected.filter((item) => item.group === group).length])),
    },
    items: selected,
  }
...
function selfTest() {
  ...
  const registry = {
    ...
    entries: [{ identity: { canonicalStrong: 'H776' }, approvedSenseTree: Array.from({ length: 26 }, (_, index) => ({ id: String(index + 1) })), evidencePacketFingerprint: 'sha256:h776' }],
  }
  const result = buildGenesisP5GoldSet(inventory, registry)
  assert.equal(result.items.length, GOLD_SIZE)
  assert.equal(new Set(result.items.map((item) => item.strong)).size, GOLD_SIZE)
  assert.equal(result.items[0].strong, GOLDEN_CONTROL)
  assert.deepEqual(result.counts.groups, GROUP_QUOTAS)
  assert.equal(result.governance.candidateGenerationAllowed, false)
...
function main() {
  ...
  console.log(`Genesis P5 Gold Set written: ${args.out}`)
  console.log(`  selected: ${result.counts.selected}`)
  console.log(`  H776 approved senses: ${result.sourceEvidence.h776ApprovedSenseCount}`)
